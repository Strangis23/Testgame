...
function love.load()
    ...
    local roadSpacing = 12
    for y = 1, MAP_HEIGHT do
        for x = 1, MAP_WIDTH do
            if map[y][x].biome.isUrban and (x % roadSpacing == 0 or y % roadSpacing == 0) then
                map[y][x].isRoad = true
            end
        end
    end
    ...
    local houseW, houseH = Images.house:getDimensions()
    local tilesW, tilesH = math.ceil(houseW / TILE_SIZE), math.ceil(houseH / TILE_SIZE)
    for y = 1, MAP_HEIGHT - tilesH do
        for x = 1, MAP_WIDTH - tilesW do
            if map[y][x].biome.isUrban then
                local isAdjacentToRoad = false
                if (map[y-1] and map[y-1][x].isRoad) or (map[y+tilesH] and map[y+tilesH][x].isRoad) or (map[y][x-1] and map[y][x-1].isRoad) or (map[y][x+tilesW] and map[y][x+tilesW].isRoad) then
                    isAdjacentToRoad = true
                end
                if isAdjacentToRoad and math.random() < 0.2 then
                    local canPlaceBuilding = true
                    for by = y, y + tilesH - 1 do
                        for bx = x, x + tilesW - 1 do
                            if (buildingGrid[by] and buildingGrid[by][bx]) or map[by][bx].isRoad then
                                canPlaceBuilding = false; break
                            end
                        end
                        if not canPlaceBuilding then break end
                    end
                    if canPlaceBuilding then
                        table.insert(placedBuildings, {x=(x-1)*TILE_SIZE, y=(y-1)*TILE_SIZE, width=houseW, height=houseH, img=Images.house})
                        for by = y, y + tilesH - 1 do
                            for bx = x, x + tilesW - 1 do
    ...
    for y = 1, MAP_HEIGHT do
        for x = 1, MAP_WIDTH do
            if not (buildingGrid[y] and buildingGrid[y][x]) then
                local tile = map[y][x]
                local scale = 1; local sizeRoll = math.random(); if sizeRoll > 0.9 then scale = 4 elseif sizeRoll > 0.6 then scale = 2 end
                if tile.biome.treeChance and math.random() < tile.biome.treeChance then table.insert(worldObjects, {type='tree', x=(x-1)*TILE_SIZE, y=(y-1)*TILE_SIZE, active=true, scale=scale}) end
                if tile.biome.stoneChance and math.random() < tile.biome.stoneChance then local rT = 'rock'; if math.random() < 0.3 then rT = 'iron_vein' end; table.insert(worldObjects, {type=rT, x=(x-1)*TILE_SIZE, y=(y-1)*TILE_SIZE, active=true, scale=scale}) end
            end
        end
    end
...
function love.keypressed(key)
        ...
        for i, obj in ipairs(worldObjects) do
            ...
            if obj.active and interactX > obj.x and interactX < obj.x + objSize and interactY > obj.y and interactY < obj.y + objSize then
                ...
                if obj.type == 'tree' or obj.type == 'rock' then
                    print("Gathered " .. itemData.name .. " got " .. yield .. " " .. itemData.drop)
                    player.inventory[itemData.drop] = player.inventory[itemData.drop] + yield
                    obj.active = false
                elseif obj.type == 'iron_vein' then
                    if player.inventory.pickaxe > 0 then
                        print("Mined " .. itemData.name .. " got " .. yield .. " " .. itemData.drop)
                        player.inventory[itemData.drop] = player.inventory[itemData.drop] + yield
                        obj.active = false
    ...
    if key == "1" then
        ...
        if player.inventory.wood >= r[1].amount and player.inventory.stone >= r[2].amount then
            player.inventory.wood = player.inventory.wood - r[1].amount
            player.inventory.stone = player.inventory.stone - r[2].amount
            player.inventory.pickaxe = player.inventory.pickaxe + 1
    ...
    if key == "2" then
        ...
        if player.inventory.stone >= r[1].amount then
    ...
    if key == "b" then
        if player.inventory.stone_wall > 0 then
            local tX = math.floor((player.x - PLAYER_COLLISION_OX + PLAYER_COLLISION_W/2) / TILE_SIZE) + 1
            local tY = math.floor((player.y - PLAYER_COLLISION_OY + PLAYER_COLLISION_H/2) / TILE_SIZE) + 1
            local k = tX .. "," .. tY
            if not builtObjects[k] and not (buildingGrid[tY] and buildingGrid[tY][tX]) then
                player.inventory.stone_wall = player.inventory.stone_wall - 1
                -- === THE FIX IS HERE ===
                -- The table was missing the '=' signs.
                builtObjects[k] = {type = 'wall', tileX = tX, tileY = tY}
...
function love.update(dt)
    ...
    pX = (pX % WORLD_PIXEL_WIDTH + WORLD_PIXEL_WIDTH) % WORLD_PIXEL_WIDTH
    pY = (pY % WORLD_PIXEL_HEIGHT + WORLD_PIXEL_HEIGHT) % WORLD_PIXEL_HEIGHT
    ...
    player.x = (player.x % WORLD_PIXEL_WIDTH + WORLD_PIXEL_WIDTH) % WORLD_PIXEL_WIDTH
    player.y = (player.y % WORLD_PIXEL_HEIGHT + WORLD_PIXEL_HEIGHT) % WORLD_PIXEL_HEIGHT
    ...
    for i, obj in ipairs(placedObjects) do
        local cX, cY = player.x - PLAYER_COLLISION_OX, player.y - PLAYER_COLLISION_OY
...
function love.draw()
    for offsetX = -1, 1 do for offsetY = -1, 1 do
        ...
        for y = sY, eY do for x = sX, eX do
            love.graphics.setColor(map[y][x].biome.color); love.graphics.rectangle("fill", (x-1)*TILE_SIZE, (y-1)*TILE_SIZE, TILE_SIZE, TILE_SIZE)
            if map[y][x].isRoad and not (buildingGrid[y] and buildingGrid[y][x]) then
